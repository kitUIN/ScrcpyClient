...
namespace ScrcpyClient
{
    ...
    public unsafe class VideoStreamDecoder : IDisposable
    {
        ...
        public IVideoFrameSink FrameSink { get; set; } = NullVideoFrameSink.Instance;
        ...
        private long currentPresentationTimestampUs = -1;
        ...
        {
            currentPresentationTimestampUs = pts;
        ...
        private void DecodePacket()
        {
            ...
            if (ret != ffmpeg.AVERROR(ffmpeg.EAGAIN))
            {
                ...
                while (ret >= 0)
                {
                    ...
                    swsContext = ffmpeg.sws_getCachedContext(swsContext, frame->width, frame->height, ctx->pix_fmt, frame->width, frame->height, AVPixelFormat.AV_PIX_FMT_BGRA, 4 /* SWS_BICUBIC */, null, null, null);
                    ...
                    if (outputSliceHeight > 0)
                    {
                        ...
                        lastFrame = new FrameData(destBufferPtr, destSize, frame->width, frame->height, FrameCount, AVPixelFormat.AV_PIX_FMT_BGRA);
                        OnFrame?.Invoke(this, lastFrame);

                        var managedCopy = GC.AllocateUninitializedArray<byte>(destSize);
                        new ReadOnlySpan<byte>(destBufferPtr, destSize).CopyTo(managedCopy);
                        FrameSink.OnFrame(new DecodedFrame(managedCopy, frame->width, frame->height, destStride[0], currentPresentationTimestampUs, FrameCount, FramePixelFormat.Bgra32));
                    }
                    else
                    {
